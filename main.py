...
for batch_file in batch_files:
    if os.path.exists(batch_file):
        ...
        # Start an MLflow run for each model and batch
        for name, model in models.items():
            with mlflow.start_run(run_name=f"{name}_{os.path.basename(batch_file)}"):
                # Train the model
                ...
                mlflow.sklearn.log_model(
                    ...
                    registered_model_name=f"{name}_model_{os.path.basename(batch_file).split('.')[0]}"
                )

                # Output the results
                print(f"Batch: {batch_file}, Run: {name}")
...
# Check results
print("All models and metrics logged successfully.")
